...
class AbstractModel
{
    ...
    {
        foreach (static::$tabelSchema as $columnName => $type) {
            if ($type == 4) {
    ...
    {
        $sql = 'INSERT INTO ' . static::$tabelName . ' SET ' . $this->buildNameParametersSQL();
    ...
    {
        $sql = 'UPDATE ' . static::$tabelName . ' SET ' . $this->buildNameParametersSQL() . ' WHERE ' . static::$primaryKey . ' = ' . $this->{static::$primaryKey};
        $stmt = DatabaseHandler::factory()->prepare($sql);
        $this->prepareValues($stmt);
        return $stmt->execute();
    }
    ...
    {
        $sql = 'DELETE FROM ' . static::$tabelName . '  WHERE ' . static::$primaryKey . ' = ' . $this->{static::$primaryKey};
        $stmt = DatabaseHandler::factory()->prepare($sql);
        return $stmt->execute();
    }
    ...
    public static function getAll()
    {
        ...
        if(method_exists(get_called_class(), '__construct')) {
            $results = $stmt->fetchAll(\PDO::FETCH_CLASS | \PDO::FETCH_PROPS_LATE, get_called_class(), array_keys(static::$tabelSchema));
        } else {
    ...
    {
        $sql = 'SELECT * FROM ' . static::$tabelName . '  WHERE ' . static::$primaryKey . ' = "' . $pk . '"';
        $stmt = DatabaseHandler::factory()->prepare($sql);
